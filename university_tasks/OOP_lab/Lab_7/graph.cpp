...
void TGraph::draw(QPainter* p, QRect r, QColor c)
{
    ...
    for (int i=0; i<tops_count; i++)
    {
        for (int j=0; j<matrix.get_matrix()[i].length(); j++)
        {
            if (i == matrix.get_matrix()[i][j] - 1)
            {
            ...
            else
            {
                ...
                angle = atan2((t[matrix.get_matrix()[i][j] - 1].center().y() - t[i].center().y()), t[matrix.get_matrix()[i][j] - 1].center().x() - t[i].center().x());
                QLineF line(t[i].center() + QPointF(rad*cos(angle), rad*sin(angle)), t[matrix.get_matrix()[i][j] - 1].center() - QPoint(rad*cos(angle), rad*sin(angle)));
                p->drawLine(line);
